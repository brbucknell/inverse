...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black font-serif">
      <div
        ...
        style={{
          backgroundImage: "url('/inverse4.png')",
        }}
      />
  ...
  <section className="nav-section flex w-full justify-center">
    <nav className="flex w-full max-w-6xl flex-wrap justify-center gap-x-6 gap-y-3 rounded-xl border border-yellow-500/20 bg-black/50 px-8 py-3 text-xl font-bold backdrop-blur-sm">
      {realms.map((realm) => (
        <button
          key={realm.name}
